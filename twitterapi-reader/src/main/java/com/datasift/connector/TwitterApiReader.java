...
public class TwitterApiReader {
     ...
     */
    private Logger log = LoggerFactory.getLogger(TwitterApiReader.class);

    /**
     * Whether the reader should retry if the client exits.
     */
    private AtomicBoolean retry = new AtomicBoolean(true);

    /**
     * Set whether the reader should retry if the client exits.
    ...
    /**
     * Get whether the reader should retry if the client exits.
    ...
    /**
     * Connects to twitter then loops until the client says it is done, reading
    ...
    public final void run(final String[] args) {
        ...
        // Create the Twitter client and buffer queue
        LinkedBlockingQueue<String> buffer =
                getBufferQueue(config.twitter.bufferSize);

        final Client client = getTwitterClient(
                buffer,
                config.twitter);
        ...
        while (this.getRetry()) {
            // If the Twitter client cannot connect it will retry with
            // a linear back-off.
            client.connect();
            readAndSend(
                    buffer,
     ...
     * @param buffer the queue which the Twitter client reads into
    ...
    protected void addShutdownHook(
        ...
        this.runtimeAddShutdownHook(new Thread() {
            public void run() {
                ...
                // Gracefully stop the Twitter client
                client.stop();
     ...
     * Loops until the Twitter client says it is done, which happens
     * when the client gets an error or it is stopped.
     * @param buffer buffers the Twitter data
    ...
    protected void readAndSend(
        ...
        while (readAndSendPredicate.process()) {
            String message = null;
            try {
                this.log.trace("Reading message from Twitter buffer");
                message = buffer.take();
                metrics.read.mark();
    ...
    /**
     * Logs the reason that the Twitter client said it was done
     ...
     */
    @SuppressWarnings("checkstyle:designforextension")
    protected Client getTwitterClient(
            final LinkedBlockingQueue<String> buffer,
            final TwitterConfig config) {

        log.info("Building Twitter client");
        ...
        // Processor
        LineStringProcessor processor = new LineStringProcessor(buffer, config.bufferTimeout);
     ...
     * @param stats The Twitter client stats tracker
