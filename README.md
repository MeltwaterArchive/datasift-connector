...
Processing of Historical PowerTrack sourced gzip archive files is supported. The Connector's historics-api component manages a database of historics jobs details. Details on how to submit a new job to the Connector are provided under the [historics-api readme](https://github.com/datasift/datasift-connector/tree/master/historics-api). The historics-reader service runs by default every five minutes, downloads any complete historics job archives, and sends the interactions contained within to Kafka for processing.
